...
*/
        ...
        StringBuilder buf =
                new StringBuilder(DateTime.Now.Ticks.ToString("x"));
        ...
        for (int i = 0; i < 128; i += 8) {
            a_in[i/8] = (int) Convert.ToInt64(
                    buf.ToString().Substring(i, 8), 16);
        }
    ...
    private String Bin2hex(int[] binarray) {
        ...
        for (int i = 0; i < binarray.Length; i++) {
            ...
            buf.Append(table[a>>16 & 0x0000000f]);
            buf.Append(table[a>>12 & 0x0000000f]);
            buf.Append(table[a>> 8 & 0x0000000f]);
            buf.Append(table[a>> 4 & 0x0000000f]);
            buf.Append(table[a     & 0x0000000f]);
        }
